...
export function SectionHeading({ id }: { id: MainViewId }) {
  return <h2 className="text-xl font-semibold tracking-tight text-zinc-950 dark:text-zinc-50">{MAIN_VIEW_LABELS[id]}</h2>;
}
...
export function ChartPanel({ title, meta, className, children }: { title?: string; meta?: string; className?: string; children: ReactNode }) {
  return <section className={cx('min-w-0 overflow-hidden bg-white dark:bg-zinc-900', className)}>
