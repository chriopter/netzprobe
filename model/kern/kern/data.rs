...
pub(super) fn berlin_date_and_hour(iso: &str) -> Result<(String, usize), SimulationError> {
...
pub(super) fn package_data(raw: &str) -> Result<Value, SimulationError> {
...
pub(super) fn value_field(raw: &str, key: &str) -> Result<String, SimulationError> {
...
pub(super) fn parse_json<T: for<'de> Deserialize<'de>>(raw: &str) -> Result<T, SimulationError> {
    serde_json::from_str(raw).map_err(|err| data_error(err.to_string()))
}

pub(super) fn s_bool(value: &Value, path: &[&str]) -> Result<bool, SimulationError> {
    walk(value, path)
        .and_then(Value::as_bool)
        .ok_or_else(|| data_error(format!("missing bool {}", path.join("."))))
}

pub(super) fn s_number(value: &Value, path: &[&str]) -> Result<f64, SimulationError> {
    walk(value, path)
        .and_then(Value::as_f64)
        .ok_or_else(|| data_error(format!("missing number {}", path.join("."))))
}

pub(super) fn path_number(value: &Value, path: &[&str]) -> Result<f64, SimulationError> {
    walk(value, path)
        .and_then(Value::as_f64)
        .ok_or_else(|| data_error(format!("missing number {}", path.join("."))))
}

pub(super) fn number(value: &Value, key: &str) -> Result<f64, SimulationError> {
    value
        .get(key)
        .and_then(Value::as_f64)
...
pub(super) fn gen_number(package_id: &str, key: &str) -> Result<f64, SimulationError> {
    number(&generation_package(package_id)?, key)
}

pub(super) fn storage_number(package_id: &str, key: &str) -> Result<f64, SimulationError> {
    number(&storage_package(package_id)?, key)
}

pub(super) fn trade_number(package_id: &str, path: &[&str]) -> Result<f64, SimulationError> {
    path_number(&trade_package(package_id)?, path)
}

pub(super) fn snap_gen(package_id: &str, value: f64) -> Result<f64, SimulationError> {
...
pub(super) fn generation_package(package_id: &str) -> Result<Value, SimulationError> {
...
pub(super) fn storage_package(package_id: &str) -> Result<Value, SimulationError> {
    package_data(match package_id {
...
pub(super) fn trade_package(package_id: &str) -> Result<Value, SimulationError> {
    package_data(match package_id {
...
pub(super) fn data_error(message: impl Into<String>) -> SimulationError {
